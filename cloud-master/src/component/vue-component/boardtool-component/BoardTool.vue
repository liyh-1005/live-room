<template>
  <div :class="['tic-player__tool', direction, { 'live-board-mode': boardliveMode }, { 'float-video-mode': floatVideo }]">
    <ul class="tic-toolbar-menu">
      <template v-for="tool in tools">
        <li :key="tool.key" v-bind:class="[tool.key == currentTool ? 'active' : '', 'tic-item-' + tool.key]" @click="toolTap(tool.key)" class="tool-item">
          <a href="javascript:;" class="tic-btn">
            <i v-bind:class="{'tic-icon--max': !isMobile, 'tic-icon--min': isMobile, [`tic-icon-${tool.key}`]: true}">
            ...
            <div class="tool-box">
              <div class="tic-toolbar-dorp toolgraphical-drop">
                <div class="tic-toolbar-dorp-list">
                  <ul>
                    <li v-for="(thin, index) in brushThin" :key="index" @click="tapBrushThin(tool.key, index, thin, $event)" :class="{ actived: brushThinIndex === index }">
                ...
                <div class="tic-toolbar-dorp-list">
                  <ul>
                    <li v-for="(type, index) in brushType" :key="index" @click="setBrushType(tool.key, index, type, $event)" :class="{ actived: brushTypeIndex === index }">
                ...
                <div class="tic-toolbar-dorp-list">
                  <ul>
                    <li v-for="(color, index) in colorList" :key="index" @click="brushColorChange(tool.key, index, color, $event)" :class="{ actived: brushColorIndex === index }">
          ...
          <template v-if="tool.key === 'toolchoose'">
            <div class="tool-box select-box">
              <div class="tic-toolbar-dorp toolchoose-drop">
                <div class="tic-toolbar-dorp-list">
                  <ul>
                    <li v-for="(select, index) in selectType" :key="index" @click.stop="setSelectType(tool.key, index, select)" :class="{ actived: selectTypeIndex === index }">
          ...
          <template v-if="tool.key === 'tooltext'">
            <div class="tool-box">
              <div class="tic-toolbar-dorp tooltext-drop">
                <div class="tic-toolbar-dorp-list">
                  <ul>
                    <li v-for="(size, index) in textSize" :key="index" @click="setTextSize(tool.key, index, size * 20)" :class="{ actived: textSizeIndex === index }">
                ...
                <div class="tic-toolbar-dorp-list">
                  <ul>
                    <li v-for="(color, index) in colorList" :key="index" @click="textColorChange(tool.key, index, color)" :class="{ actived: textColorIndex === index }">
                      <div class="bg-color-border">
                        <span class="bg-color" :style="'background-color:' + color"></span>
                      </div>
                    </li>
...
export default {
  ...
  data() {
    return {
      isMobile: TCIC.TSession.instance.isMobile(),
  ...
  methods: {
    ...
    updateVideoMode(isFloat) {
      this.floatVideo = isFloat;
    },
    toggleToolsPanel() {
    ...
    toolTap(type) {
      var activeStatus = ["toolchoose", "toolrubber", "tooltext", "toollaser", "tooldrag"];
    ...
    // 设置笔刷粗细
    tapBrushThin(type, index, thin, event) {
      if (event) {
      ...
      this.setBrushThin(thin);
    },

    setBrushThin(thin) {
      let thinVal = 50;
      if (!thin) {
        thin = this.brushThin[this.brushThinIndex];
      }
      switch (thin) {
        case "size-4":
          thinVal = 25;
      ...
      this.EventBus.$emit("setting_board_brush_thin", thinVal);
    },
...
.live-board-mode.float-video-mode {
  top: 130px;
}
